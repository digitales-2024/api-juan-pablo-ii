...
export function validateChanges<T extends object>(
  ...
  console.log(`[validateChanges] Validando cambios en DTO: ${JSON.stringify(dto, null, 2)}`);
  console.log(`[validateChanges] Datos actuales: ${JSON.stringify(currentData, null, 2)}`);
  ...
  for (const [key, newValue] of Object.entries(dto)) {
    ...
    if (newValue !== undefined && key in currentData) {
      ...
      console.log(`[validateChanges] Comparando campo '${key}': nuevo valor = ${JSON.stringify(newValue)}, valor actual = ${JSON.stringify(currentValue)}`);
