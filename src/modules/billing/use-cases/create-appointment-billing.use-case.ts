  ...
  async execute(
    ...
    return await this.orderRepository.transaction(async () => {
      ...
      if (appointment.status !== 'PENDING') {
        this.logger.warn(
          `Intento de generar orden para cita ${appointmentId} con estado ${appointment.status}`,
        );
        throw new BadRequestException(
          `No se puede generar una orden para una cita que no está en estado PENDING. Estado actual: ${appointment.status}`,
        );
      }
      ...
      if (!patient) {
        throw new BadRequestException(
          `Paciente con ID ${appointment.patientId} no encontrado`,
        );
      }

      // Crear metadata vacía usando el generador
      const metadata = this.appointmentGenerator.createEmptyMetadata(
        createDto,
        patient,
      );
      ...
      metadata.patientDetails.fullName =
        `${patient.name} ${patient.lastName || ''}`.trim(); // Nombre completo
      ...
      metadata.orderDetails.appointmentStart = appointment.start
        ? appointment.start.toString()
        : '';
      metadata.orderDetails.appointmentEnd = appointment.end
