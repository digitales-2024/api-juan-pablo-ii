...
} from '../dto/create-medical-prescription-billing.dto';
...
export class CreateMedicalPrescriptionUseCase {
  ...
  async execute(
    ...
    return await this.orderRepository.transaction(async () => {
      ...
      const unavailableProducts = await this.checkProductsAvailability(
        createDto.products,
      );
      ...
      const appointmentsData = await this.validateAndGetAppointments(
        createDto.appointmentIds,
      );

      // Crear metadata vacía usando el generador
      const metadata =
        this.prescriptionGenerator.createEmptyMetadata(createDto);

      // Llenar los detalles del paciente
      metadata.patientDetails.fullName =
      ...
      const productTotals = await this.calculateProductTotals(
        createDto.products,
      );

      // Calcular el subtotal, impuestos y detalles de las citas
      const appointmentTotals =
        await this.calculateAppointmentTotals(appointmentsData);
      ...
      metadata.orderDetails.products = productTotals.productDetails.map(
        (product) => ({
          id: product.id,
          name: product.name,
      ...
      metadata.orderDetails.services = await Promise.all(
        appointmentsData.map(async (appointment) => {
          const serviceData = await this.serviceService.findOne(
          ...
          return {
            ...
            name: serviceData.name ?? 'Consulta médica',
            quantity: 1,
          };
        }),
      );
      ...
      if (appointmentsData.length > 0 && appointmentsData[0].staff) {
        metadata.orderDetails.staffId = appointmentsData[0].staff.id;
      }
   ...
   */
  private async checkProductsAvailability(
