...
export class AppointmentGenerator extends BaseOrderGenerator {
     ...
     */
    async calculateTotal(input: CreateMedicalAppointmentBillingDto): Promise<number> {
        const appointmentId = input.appointmentId;
        return this.appointmentService.getServicePriceByAppointmentId(appointmentId);
    }

    createEmptyMetadata(createDto: CreateMedicalAppointmentBillingDto, patient: Patient): MedicalAppointmentMetadata {
        return {
            patientDetails: {
                fullName: '', // Se llenará más tarde
                dni: '',
